...
* DONE some of the dungeons and stuff are missing regions
...
* DONE pets are slow when told to follow
...
* VERIFY missing mana potions -> i.e. make pots that are craftable via alchemy that restore mana
  Need to test crafting and double check naming
...
* DONE ebless and other spells not working??
* DONE warriors seem not to be able to damage each other
...
* DONE make it so spec crafter is required to have a chance at exceptional for anything above dripstone
* DONE seems that people can cast spells holding shields and weapons
...
* DONE Add EquippedCasting true for chaos/order.
* DONE only str mods drop, no dex or int
* VERIFY rangers get fucked on maxskills for spec because tactics
* DONE kill spell is OP
* VERIFY armor/weapons are not displaying mod titles properly (onsingleclick)
* DONE Earth/Necro scrolls are right colour
* DONE Casting with weapons or not.
* TODO comb through "artifacts" (see treasure chest system) and see which ones are appropriate to drop in high level chests
  i.e. Arctic Death Dealer does water damage or something... does BaseWeapon.Damage or OnHit support this?
* DONE tmaps not dropping skill mods, etc.
* DONE easter egg chest (make sure to check reload from save)
* TODO guys are reporting can't gain taming off same animal more than once?
