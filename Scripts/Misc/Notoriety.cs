...
namespace Server.Misc
{
    public class NotorietyHandlers
	{
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
			...
			{
				if( !target.Body.IsHuman && !target.Body.IsGhost && !IsPet( target as BaseCreature ) && !(target is PlayerMobile) || !target.CanBeginAction( typeof( Spells.Seventh.PolymorphSpell ) ) )
					return Notoriety.CanBeAttacked;
			}
