...
namespace Server.Misc
{
	...
	{
		private enum DefaultClientResponse
		{
			Ignore,
			Warn,
			Annoy,
		...
		private static DefaultClientResponse m_DefaultClientResponse = DefaultClientResponse.LenientKick;
		...
		public static void Initialize()
		{
			...
			if( Required != null )
			{
				...
				Console.WriteLine( "Restricting client version to {0}. Action to be taken: {1}", Required, m_DefaultClientResponse );
				Utility.PopColor();
			}
		}
		...
		private static void EventSink_ClientVersionReceived( ClientVersionReceivedArgs e )
		{
			...
			if( Required != null && version < Required && ( m_DefaultClientResponse == DefaultClientResponse.Kick ||m_DefaultClientResponse == DefaultClientResponse.LenientKick && DateTime.Now - state.Mobile.CreationTime > m_AgeLeniency && state.Mobile is PlayerMobile && ((PlayerMobile)state.Mobile).GameTime > m_GameTimeLeniency))
			{
			...
			{
				switch( m_DefaultClientResponse )
				{
					case DefaultClientResponse.Warn:
					{
					...
					case DefaultClientResponse.LenientKick:
					case DefaultClientResponse.Annoy:
					{
			...
			if( m.NetState != null && m.NetState.Version < Required )
			{
				Gump g = new WarningGump( 1060637, 30720,
					...
					delegate( Mobile mob, bool selection, object o )
					{
						...
						if ( m_DefaultClientResponse == DefaultClientResponse.LenientKick )
