...
namespace Server.RemoteAdmin
{
    public class AdminNetwork
	{
		...
		public static void OnConsoleString( string str )
		{
			...
			if ( m_NewLine )
			{
				outStr = $"[{DateTime.Now.ToString(DateFormat)}]: {str}";
				m_NewLine = false;
			}
			else
			{
		...
		public static void OnConsoleChar( char ch )
		{
			...
			{
				string outStr;
				outStr = $"[{DateTime.Now.ToString(DateFormat)}]: {ch}";
		...
		public static void OnConsoleLine( string line )
		{
			string outStr;
			if ( m_NewLine )
				outStr = $"[{DateTime.Now.ToString(DateFormat)}]: {line}{Console.Out.NewLine}";
			else
		...
		public static void OnReceive( NetState state, PacketReader pvSrc )
		{
			...
			{
				string statStr =
					$", Name={Misc.ServerList.ServerName}, Age={(int) (DateTime.Now - Items.Clock.ServerStart).TotalHours}, Clients={NetState.Instances.Count}, Items={World.Items.Count}, Chars={World.Mobiles.Count}, Mem={(int) (GC.GetTotalMemory(false) / 1024)}K, Ver={ProtocolVersion}";
