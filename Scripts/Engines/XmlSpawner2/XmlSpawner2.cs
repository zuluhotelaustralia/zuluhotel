...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("XmlSaveOld", DiskAccessLevel, new CommandEventHandler(SaveDefault_OnCommand));
        ...
        public static void XmlUnLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
        {
                ...
                if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                    {
                        ...
                        if (SpawnerPrefix == null || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
                        {
                            ...
                            XmlSpawner DefaultSpawner = null;
                            foreach (Item i in World.Items.Values)
                            {
                                if (i is XmlSpawner)
                                {
                                    ...
                                    {
                                        DefaultSpawner = (XmlSpawner)i;
                                        if (DefaultSpawner != null)
                                        {
                                            spawners_deleted++;
                                            DefaultSpawner.Delete();
                                        }
            ...
            if (from != null)
                from.SendMessage("{0}/{8} spawner(s) were unloaded using file {1} [Trammel={2}, Felucca={3}, Other={4}].",
                    spawners_deleted, filename, TrammelCount, FeluccaCount, OtherCount, TotalCount);
                ...
                try
                {
                    ...
                    using (StreamReader sr = new StreamReader(filename))
                    {
                        ...
                        while ((line = sr.ReadLine()) != null)
                        {
                            ...
                            {
                                ParseDefaultMapFormat(from, filename, line, args, linenumber, ref spawnercount, ref badspawnercount, ref overridemap, ref overridemintime, ref overridemaxtime);
                            }
        ...
        private static void ParseDefaultMapFormat(Mobile from, string filename, string line, string[] args, int linenumber, ref int spawnercount, ref int badspawnercount, ref int overridemap, ref double overridemintime, ref double overridemaxtime)
        {
            // format of each .map line is * Dragon:Wyvern 5209 965 -40 2 2 10 50 30 1
            //  * typename:typename:... x y z map mindelay maxdelay homerange spawnrange maxcount
            // or
        ...
        public static void XmlLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, Point3D fromloc, Map frommap, bool loadrelative, int maxrange, bool loadnew, out int processedmaps, out int processedspawners, bool verbose)
        {
            ...
            if (ds.Tables != null && ds.Tables.Count > 0)
            {
                ...
                if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
                    foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                    {
                        ...
                        if (found_spawner && DefaultSpawner != null && !DefaultSpawner.Deleted)
                        {
                            ...
                            if (setObjectName != null && setObjectName.Length > 0)
                            {
                                ...
                                if (loadnew)
                                {
                                    ...
                                    DefaultSpawner.m_SetPropertyItem = BaseXmlSpawner.FindItemByName(null, tmpsetObjectName, typestr);
                                }
                                // if this fails then try the original
                                if (DefaultSpawner.m_SetPropertyItem == null)
                                {
                                    DefaultSpawner.m_SetPropertyItem = BaseXmlSpawner.FindItemByName(null, namestr, typestr);
                                }
                                if (DefaultSpawner.m_SetPropertyItem == null)
                                {
                                    failedsetitemcount++;
                                    if (from != null)
                                        from.SendMessage(33, "Failed to initialize SetItemProperty Object '{0}' on ' '{1}' at [{2} {3}] in {4}",
                                            setObjectName, DefaultSpawner.Name, DefaultSpawner.Location.X, DefaultSpawner.Location.Y, DefaultSpawner.Map);
                                    // log it
                                    try
                                    {
                                        using (StreamWriter op = new StreamWriter("badxml.log", true))
                                        {
                                            ...
                                            op.WriteLine("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}",
                                                setObjectName, DefaultSpawner.Location.X, DefaultSpawner.Location.Y, DefaultSpawner.Location.Z, DefaultSpawner.Map, DefaultSpawner.Name, filename);
                                            op.WriteLine();
                                        }
                                    }
                                    catch { }
                            ...
                            if (triggerObjectName != null && triggerObjectName.Length > 0)
                            {
                                ...
                                if (loadnew)
                                {
                                    ...
                                    DefaultSpawner.m_ObjectPropertyItem = BaseXmlSpawner.FindItemByName(null, tmptriggerObjectName, typestr);
                                }
                                // if this fails then try the original
                                if (DefaultSpawner.m_ObjectPropertyItem == null)
                                {
                                    DefaultSpawner.m_ObjectPropertyItem = BaseXmlSpawner.FindItemByName(null, namestr, typestr);
                                }
                                if (DefaultSpawner.m_ObjectPropertyItem == null)
                                {
                                    ...
                                    if (from != null)
                                        from.SendMessage(33, "Failed to initialize TriggerObject '{0}' on ' '{1}' at [{2} {3}] in {4}",
                                            triggerObjectName, DefaultSpawner.Name, DefaultSpawner.Location.X, DefaultSpawner.Location.Y, DefaultSpawner.Map);
                                    // log it
                                    try
                                    {
                                        using (StreamWriter op = new StreamWriter("badxml.log", true))
                                        {
                                            op.WriteLine("# Failed TriggerObject initialization : {0}: Format: ObjectName X Y Z Map SpawnerName Xmlfile",
                                                DateTime.Now);
                                            op.WriteLine("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}",
                                                triggerObjectName, DefaultSpawner.Location.X, DefaultSpawner.Location.Y, DefaultSpawner.Location.Z, DefaultSpawner.Map, DefaultSpawner.Name, filename);
                                            op.WriteLine();
                                        }
                                    }
                                    catch { }
            ...
            if (from != null)
                from.SendMessage("{0} spawner(s) were created from file {1} [Trammel={2}, Felucca={3}, Other={4}].",
                    TotalCount, filename, TrammelCount, FeluccaCount, OtherCount);
        ...
        public static void SaveDefault_OnCommand(CommandEventArgs e)
        {
        ...
        public static bool SaveSpawnList(Mobile from, ArrayList savelist, string dirname, Stream stream, bool oldformat, bool verbose)
        {
            ...
            if (from != null)
            {
                from.SendMessage("{0} spawner(s) were saved to file {1} [Trammel={2}, Felucca={3}, Other={4}].",
                    TotalCount, dirname, TrammelCount, FeluccaCount, OtherCount);
            }
            return true;
