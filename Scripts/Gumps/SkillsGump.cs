...
namespace Server.Gumps
{
	...
	{
		public static readonly bool DefaultStyle = PropsConfig.DefaultStyle;
		...
		public EditSkillGump( Mobile from, Mobile target, Skill skill, SkillsGumpGroup selected ) : base( GumpOffsetX, GumpOffsetY )
		{
			...
			AddImageTiled( BorderSize, BorderSize, TotalWidth - (DefaultStyle ? SetWidth + OffsetSize : 0), TotalHeight, OffsetGumpID );
	...
	{
		public static bool DefaultStyle = PropsConfig.DefaultStyle;
		...
		/*
		private static bool PrevLabel = DefaultStyle, NextLabel = DefaultStyle;
		...
		public SkillsGump( Mobile from, Mobile target, SkillsGumpGroup selected ) : base( GumpOffsetX, GumpOffsetY )
		{
			...
			int emptyWidth = TotalWidth - PrevWidth - NextWidth - OffsetSize * 4 - (DefaultStyle ? SetWidth + OffsetSize : 0);
			...
			if ( !DefaultStyle )
				AddImageTiled( x - (DefaultStyle ? OffsetSize : 0), y, emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0), EntryHeight, HeaderGumpID );
			...
			for ( int i = 0; i < m_Groups.Length; ++i )
			{
				...
				x -= DefaultStyle ? OffsetSize : 0;

				AddImageTiled( x, y, emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0), EntryHeight, EntryGumpID );
				AddLabel( x + TextOffsetX, y, TextHue, group.Name );

				x += emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0);
				x += OffsetSize;
				...
				if ( group == selected )
				{
					...
					for ( int j = 0; j < group.Skills.Length; ++j )
					{
						...
						AddImageTiled( x, y, emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0) - OffsetSize - IndentWidth, EntryHeight, EntryGumpID );
						AddLabel( x + TextOffsetX, y, TextHue, sk == null ? "(null)" : sk.Name );

						x += emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0) - OffsetSize - IndentWidth;
						x += OffsetSize;
