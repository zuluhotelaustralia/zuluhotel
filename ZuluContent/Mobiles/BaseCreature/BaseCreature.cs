...
namespace Server.Mobiles
{
  public partial class BaseCreature : Mobile
  {
    ...
    {
      if (m_AI != null)
        m_AI.NextMove = Core.TickCount + (int)TimeSpan.FromSeconds(BreathStallTime).TotalMilliseconds;
    }
    ...
    public virtual void OnThink()
    {
      ...
      if ((CanHeal || CanHealOwner) && Alive && !IsHealing && !BardPacified)
      {
            ...
            InLOS(owner) && owner.Hits < HealOwnerTrigger * owner.HitsMax)
        {
          ...
          m_NextHealOwnerTime = Core.TickCount + (int)TimeSpan.FromSeconds(HealOwnerInterval).TotalMilliseconds;
        }
