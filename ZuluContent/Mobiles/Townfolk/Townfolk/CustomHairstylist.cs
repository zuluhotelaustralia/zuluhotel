...
namespace Server.Mobiles
{
    ...
    public class ChangeHairstyleGump : Gump
    {
        ...
        public ChangeHairstyleGump(Mobile from, Mobile vendor, int price, bool facialHair,
            ChangeHairstyleEntry[] entries) : base(50, 50)
        {
            m_From = from;
            m_Vendor = vendor;
            m_Price = price;
            ...
            if (info.ButtonID == 1)
            {
                ...
                if (switches.Length > 0)
                {
                    ...
                    if (index >= 0 && index < m_Entries.Length)
                    {
                        ...
                        if (m_From is PlayerMobile player) player.RemoveHairMods();
