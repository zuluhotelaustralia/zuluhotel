  ...
  public class Weather
  {
      ...
      int moveSpeed, int width, int height, Rectangle2D bounds)
    {
      for (int i = 0; i < m_Facets.Length; ++i)
      {
        ...
        Weather w = new Weather(m_Facets[i], new[] {area}, temperature, chanceOfPercipitation,
          chanceOfExtremeTemperature, TimeSpan.FromSeconds(30.0));
    ...
    public virtual void Reposition()
    {
      ...
      for (int j = 0; j < 10; ++j)
      {
        area = new Rectangle2D(m_Bounds.X + Utility.Random(m_Bounds.Width - width),
          m_Bounds.Y + Utility.Random(m_Bounds.Height - height), width, height);
