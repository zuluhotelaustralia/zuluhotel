...
namespace Server.Misc
{
    public class AutoSave : Timer
    {
        ...
        private static void Backup()
        {
            ...
            {
                DirectoryInfo dir = Match(existing, Backups[i]);
                ...
                if (i > 0)
                {
                    ...
                    if (timeStamp != null)
                    {
                        try
                        {
                            dir.MoveTo(FormatDirectory(root, Backups[i - 1], timeStamp));
                        }
                        catch
                        {
                        }
            ...
            if (Directory.Exists(saves))
                Directory.Move(saves, FormatDirectory(root, Backups[Backups.Length - 1], GetTimeStamp()));
        }
