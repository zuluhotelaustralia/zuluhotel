...
namespace Server.Commands.Generic
{
  ...
  public class TeleCommand : BaseCommand
  {
    ...
    public override void Execute(CommandEventArgs e, object obj)
    {
      ...
      if (!from.Hidden)
      {
        Effects.SendLocationParticles(EffectItem.Create(fromLoc, from.Map, EffectItem.DefaultDuration), 0x3728, 10,
          10, 2023);
        Effects.SendLocationParticles(EffectItem.Create(toLoc, from.Map, EffectItem.DefaultDuration), 0x3728, 10, 10,
          5023);
  ...
  public class KillCommand : BaseCommand
  {
    ...
    public override void Execute(CommandEventArgs e, object obj)
    {
      ...
      else
      {
        if (!mob.Alive)
        {
          CommandLogging.WriteLine(from, "{0} {1} resurrecting {2}", from.AccessLevel, CommandLogging.Format(from),
            CommandLogging.Format(mob));
