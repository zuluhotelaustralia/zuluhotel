    ...
    public class SignGenerator
    {
        ...
        public static void Generate()
        {
            ...
            if (File.Exists(cfg))
            {
                ...
                World.Broadcast(0x35, true, "Generating signs, please wait.");
                using (StreamReader ip = new StreamReader(cfg))
                {
                    ...
                    while ((line = ip.ReadLine()) != null)
                    {
                        ...
                        SignEntry e = new SignEntry(
                            ...
                            new Point3D(Utility.ToInt32(split[2]), Utility.ToInt32(split[3]),
                                Utility.ToInt32(split[4])),
                ...
                for (int i = 0; i < list.Count; ++i)
                {
                    ...
                    for (int j = 0; maps?.Length > j; ++j)
                        AddStatic(e.m_ItemID, e.m_Location, maps[j], e.m_Text);
                }

                World.Broadcast(0x35, true, "Sign generation complete.");
            }
            else
            {
                World.Broadcast(0x35, true, "{0} not found.", cfg);
            }
        }

        public static void AddStatic(int itemID, Point3D location, Map map, string name)
        {
