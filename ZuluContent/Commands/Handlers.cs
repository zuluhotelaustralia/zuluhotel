...
namespace Server.Commands
{
    public class CommandHandlers
    {
        ...
        {
            CommandSystem.Prefix = ServerConfiguration.GetSetting("commandsystem.prefix", "[");
        ...
        private static void PlaySound(Mobile m, int index, bool toAll)
        {
            ...
            CommandLogging.WriteLine(m, "{0} {1} playing sound {2} (toAll={3})", m.AccessLevel,
                CommandLogging.Format(m),
                index, toAll);

            foreach (var state in m.GetClientsInRange(12))
                if (toAll || state.Mobile.CanSee(m))
                    state.SendSoundEffect(index, m.Location);
        }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < Map.AllMaps.Count; ++i)
                        {
                            ...
                            if (InsensitiveStringHelpers.Equals(name, map.Name))
                            {
                                from.Map = map;
                                return;
                            }
                        ...
                        foreach (KeyValuePair<string, Region> kvp in list)
                        {
                            ...
                            if (InsensitiveStringHelpers.Equals(r.Name, name))
                            {
            ...
            else if (e.Length == 6)
            {
                ...
                if (map != null)
                {
                    Point3D p = Sextant.ReverseLookup(map, e.GetInt32(3), e.GetInt32(0), e.GetInt32(4), e.GetInt32(1),
                        InsensitiveStringHelpers.Equals(e.GetString(5), "E"),
                        InsensitiveStringHelpers.Equals(e.GetString(2), "S"));
                    ...
                    public override void OnResponse(NetState state, int index)
                    {
                        if (index == 0)
                        {
                            CommandLogging.WriteLine(state.Mobile, "{0} {1} moving equipment item {2} of {3}",
                                state.Mobile.AccessLevel, CommandLogging.Format(state.Mobile),
        ...
        private class DismountTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                {
                    CommandLogging.WriteLine(from, "{0} {1} dismounting {2}", from.AccessLevel,
