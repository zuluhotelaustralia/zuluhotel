...
namespace Server.Commands
{
    public class Add
    {
        ...
        private static readonly Type ConstructibleType = typeof(ConstructibleAttribute);

        private static readonly Type EnumType = typeof(Enum);

        private static readonly Type TypeType = typeof(Type);

        private static readonly Type ParsableType = typeof(ParsableAttribute);

        private static readonly Type[] ParseTypes = {typeof(string)};
        private static readonly object[] ParseArgs = new object[1];
        ...
        public static int BuildObjects(Mobile from, Type type, Point3D start, Point3D end, string[] args,
            string[,] props,
                ...
                for (var i = 0; i < realProps.Length; ++i)
                {
                    ...
                    else
                    {
                        ...
                        if (attr == null)
                            ...
                            from.SendMessage("Setting this property ({0}) requires at least {1} access level.",
                                propName,
        ...
        private static void InternalRXYZ_OnCommand(CommandEventArgs e, bool outline)
        {
            if (e.Length >= 6)
            {
                var p = new Point3D(e.Mobile.X + e.GetInt32(0), e.Mobile.Y + e.GetInt32(1),
        ...
        private static void InternalZ_OnCommand(CommandEventArgs e, bool outline)
        {
            ...
            if (e.Length >= 2)
            {
                ...
                BoundingBoxPicker.Begin(from, (map, start, end) =>
                    TileBox_Callback(from, start, end,
        ...
        {
            return EntityType.IsAssignableFrom(t);
        }

        public static bool IsConstructible(ConstructorInfo ctor, AccessLevel accessLevel)
        {
            var attrs = ctor.GetCustomAttributes(ConstructibleType, false);
        ...
        {
            return type.IsSubclassOf(EnumType);
        }

        public static bool IsType(Type type)
        {
            return type == TypeType || type.IsSubclassOf(TypeType);
        }

        public static bool IsParsable(Type type)
        {
            return type.IsDefined(ParsableType, false);
        }
        ...
        public static object ParseParsable(Type type, string value)
        {
            ...
            return method?.Invoke(null, ParseArgs);
        }
