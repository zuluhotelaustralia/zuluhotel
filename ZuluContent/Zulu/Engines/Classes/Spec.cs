...
namespace Scripts.Zulu.Engines.Classes
{
  ...
  public class Spec
  {
    ...
    public static Spec GetSpec(Mobile m)
    {
      Spec spec = m switch
      {
        PlayerMobile pm => pm.Spec,
        BaseCreature bc => bc.Spec,
        _ => null
      };
    ...
    private class InternalTarget : Target
    {
      ...
      {
        Spec spec = @from switch
        {
          PlayerMobile mobile => mobile.Spec,
          BaseCreature bc => bc.Spec,
          _ => null
        };

        spec?.ComputeSpec();
        from.SendMessage("{0}: {1}, level {2}", from.Name, spec?.SpecName, spec?.SpecLevel);
      }
    }
  }
}
