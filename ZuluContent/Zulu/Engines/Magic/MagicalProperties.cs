...
namespace ZuluContent.Zulu.Engines.Magic
{
     ...
     */
    public class MagicalProperties
    {
        ...
        public TOutput GetAttr<TOutput>(MagicProp propertyType) where TOutput : unmanaged =>
            this[propertyType] is MagicAttribute<TOutput> instanceMod ? instanceMod.Target : default;
        ...
        public T GetAttr<T>() where T : unmanaged, Enum =>
            GetAttr<T>(ToPropType<T>());

        public void SetAttr<T>(T value) where T : unmanaged, Enum =>
            SetAttr(ToPropType<T>(), value);

        public TValue GetAttr<TKey, TValue>() where TKey : unmanaged, Enum where TValue : unmanaged =>
            GetAttr<TValue>(ToPropType<TKey>());
        ...
        public static MagicalProperties Deserialize(IGenericReader reader)
        {
            var mp = new MagicalProperties();

            var version = reader.ReadByte();
            var length = reader.ReadInt();
            for (var i = 0; i < length; i++)
            {
                var value = IMagicValue.Deserialize(reader);
