    ...
    public class WhoGump : Gump
    {
        ...
        public static List<Mobile> BuildList(Mobile owner, string filter)
        {
            ...
            foreach (var ns in TcpServer.Instances)
            {
                ...
                {
                    if (filter != null &&
                        (m.Name == null || m.Name.ToLower().IndexOf(filter, StringComparison.Ordinal) < 0))
                        continue;
        ...
        public void Initialize(int page)
        {
            ...
            AddImageTiled(BorderSize, BorderSize, TotalWidth - (DefaultStyle ? SetWidth + OffsetSize : 0), totalHeight,
                OffsetGumpID);
            ...
            int emptyWidth = TotalWidth - PrevWidth - NextWidth - OffsetSize * 4 -
                             (DefaultStyle ? SetWidth + OffsetSize : 0);

            if (!DefaultStyle)
                AddImageTiled(x - (DefaultStyle ? OffsetSize : 0), y, emptyWidth + (DefaultStyle ? OffsetSize * 2 : 0),
            ...
            for (int i = 0, index = page * EntryCount; i < EntryCount && index < m_Mobiles.Count; ++i, ++index)
            {
                ...
                AddLabelCropped(x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, GetHueFor(m),
