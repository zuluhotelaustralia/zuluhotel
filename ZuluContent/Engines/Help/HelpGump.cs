                ...
                case 2: // Stuck
                    {
                        ...
                        else if (from.Region.IsPartOf<Regions.JailRegion>())
                        {
                            from.SendLocalizedMessage(1114345, "", 0x35); // You'll need a better jailbreak plan than that!
                        }
                        else if (from is PlayerMobile && from.Region.CanUseStuckMenu(from) && !CheckCombat(from) && !from.Frozen && !from.Criminal)
                        {
                ...
                case 9: // Young player transport
                    {
                        if (IsYoung(from))
                        {
                            if (from.Region.IsPartOf<Regions.JailRegion>())
                            {
                                from.SendLocalizedMessage(1114345, "", 0x35); // You'll need a better jailbreak plan than that!
                            }
