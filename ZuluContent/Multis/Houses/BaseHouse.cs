...
namespace Server.Multis
{
    public abstract class BaseHouse : BaseMulti
    {
        ...
        {
            get { return TimeSpan.FromDays(5.0); }
        }
        ...
        public DecayLevel GetDefaultDecayLevel()
        {
        ...
        private List<Mobile> m_PlayerVendors = new List<Mobile>();
        private List<Mobile> m_PlayerBarkeepers = new List<Mobile>();

        private List<Item> m_LockDowns;
        private List<SecureInfo> m_Secures;

        private List<Item> m_Addons;
        ...
        public BaseHouse(int multiID, Mobile owner, int MaxLockDown, int MaxSecure) : base(multiID)
        {
            ...
            m_Doors = new List<BaseDoor>();
            m_LockDowns = new List<Item>();
                ...
                if (item is Container && !(item is BaseBoard))
                {
                    ...
                    for (int j = 0; j < children.Count; ++j)
                    {
                        ...
                        if (child.Decays && !child.IsLockedDown && !child.IsSecure &&
                            child.LastMoved + child.DecayTime <= DateTime.Now)
        ...
        public override void OnAfterDelete()
        {
            ...
            if (m_Addons != null)
            {
                for (int i = 0; i < m_Addons.Count; ++i)
                {
                    ...
                    if (item != null)
                    {
                        if (!item.Deleted && item is IAddon addonItem)
                        {
                            var deed = addonItem.Deed;

                            if (deed != null)
                            {
                                if (item is BaseAddon baItem && deed is BaseAddonDeed addonDeed)
                                {
                                    addonDeed.Mark = baItem.Components[0].Mark;
                                    addonDeed.Resource = baItem.Components[0].Resource;
                                    addonDeed.PlayerConstructed = baItem.Components[0].PlayerConstructed;
                                    addonDeed.Crafter = baItem.Components[0].Crafter;
                                }
        ...
        public virtual Guildstone FindGuildstone()
        {
            ...
            IPooledEnumerable eable =
                map.GetItemsInBounds(new Rectangle2D(X + mcl.Min.X, Y + mcl.Min.Y, mcl.Width, mcl.Height));
