    ...
    public partial class BloodyBandage : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from.InRange(GetWorldLocation(), Range))
            {
                ...
                from.SendSuccessMessage("Select a water source to clean them.");
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(BloodyBandage bandage) : base(BloodyBandage.Range, true, TargetFlags.None)
            {
                m_Bandage = bandage;
            }
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is LandTarget landTarget && Validate(landTarget.TileID) ||
                    targeted is StaticTarget staticTarget && Validate((staticTarget.ItemID & 0x3FFF) | 0x4000) ||
                    targeted is IWaterSource)
                {
                    var amount = m_Bandage.Amount / 2;
                    m_Bandage.Delete();
                    from.PlaySound(0x011);
                    from.Animate(6, 5, 1, true, false, 0);
                    from.Backpack.TryDropItem(from, new Bandage(amount), true, true);
                }
                else
                {
            ...
            private bool Validate(int tileID)
            {
                ...
                for (var i = 0; !contains && i < waterTiles.Length; i += 2)
                    contains = tileID >= waterTiles[i] && tileID <= waterTiles[i + 1];
