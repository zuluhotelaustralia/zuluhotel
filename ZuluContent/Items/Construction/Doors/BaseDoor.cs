...
namespace Server.Items
{
    public abstract class BaseDoor : Item, ILockable, ITelekinesisable, ILockpickable
    {
        ...
        private static readonly Point3D[] Offsets =
        {
            new(-1, 1, 0),
            new(1, 1, 0),
            new(-1, 0, 0),
        ...
        private static void ChainLink_OnTarget(Mobile from, object targeted, object state)
        {
            ...
            if (door == null)
            {
                from.BeginTarget(-1, false, TargetFlags.None, ChainLink_OnTarget, state);
                from.SendMessage("That is not a door. Try again.");
            }
            else
            {
        ...
        public virtual void Use(Mobile from)
        {
            if (Locked && !m_Open && UseLocks())
            {
                ...
                {
                    from.LocalOverheadMessage(MessageType.Regular, 0x3B2,
                        501282); // You quickly unlock, open, and relock the door
                }
                else if (IsInside(from))
                {
                    from.LocalOverheadMessage(MessageType.Regular, 0x3B2,
                        501280); // That is locked, but is usable from the inside.
                }
                else
                {
