    ...
    public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
    {
        ...
        public virtual void Serialize(GenericWriter writer)
        {
            ...
            ImplFlag implFlags = m_Flags & (ImplFlag.Visible | ImplFlag.Movable | ImplFlag.Stackable |
                                            ImplFlag.Insured | ImplFlag.PayedInsurance | ImplFlag.QuestItem);
        ...
        public virtual void UpdateTotal(Item sender, TotalType type, int delta)
        {
            if (!IsVirtualItem)
            {
                if (m_Parent is Item)
                    ...
                    (HeldBy as Mobile).UpdateTotal(sender, type, delta);
            }
        }
        ...
        {
            get { return (int) Math.Ceiling(Weight * Amount); }
        }
        ...
        public virtual void AddItem(Item item)
        {
            ...
            {
                Console.WriteLine("Warning: Adding item to itself: [0x{0:X} {1}].AddItem( [0x{2:X} {3}] )",
                        ...
                        if (stc != null)
                        {
                            ...
                            if (st != null)
                            {
                                ...
                                if (tradeRecip != null)
                                {
                                    ...
                                    if (ns != null)
                                    {
                                        if (tradeRecip.CanSee(this) &&
                                            tradeRecip.InRange(worldLoc, GetUpdateRange(tradeRecip)))
                                        {
                                            if (ns.ContainerGridLines)
                                                ns.Send(new ContainerContentUpdate6017(this));
                                            else
        ...
        public void PublicOverheadMessage(MessageType type, int hue, int number, string args)
        {
            if (m_Map != null)
            {
                ...
                foreach (NetState state in eable)
                {
                    ...
                    {
                        if (p == null)
                            p = Packet.Acquire(new MessageLocalized(m_Serial, m_ItemID, type, hue, 3, number, Name,
                                args));
            ...
            if (ns != null)
            {
                if (Name == null)
                {
                    if (m_Amount <= 1)
                        ...
                        ns.Send(new MessageLocalizedAffix(m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, LabelNumber,
                            "", AffixType.Append,
                            $" : {m_Amount}", ""));
                }
                else
                {
                    ns.Send(new UnicodeMessage(m_Serial, m_ItemID, MessageType.Label, 0x3B2, 3, "ENU", "",
