...
namespace Server
{
    ...
    public delegate Container CreateCorpseHandler(Mobile from, HairInfo hair, FacialHairInfo facialhair,
        List<Item> initialContent, List<Item> equipedItems);
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        private void CheckAggrExpire()
        {
            for (int i = m_Aggressors.Count - 1; i >= 0; --i)
            {
                ...
                if (info.Expired)
                {
                    ...
                    if (m_NetState != null && CanSee(attacker) &&
            ...
            for (int i = m_Aggressed.Count - 1; i >= 0; --i)
            {
                ...
                if (info.Expired)
                {
                    ...
                    if (m_NetState != null && CanSee(defender) &&
        ...
        {
            get { return Alive && (RegenThroughPoison || !Poisoned); }
        }
        ...
        private class CombatTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (DateTime.Now > m_Mobile.m_NextCombatTime)
                {
                    ...
                    if (combatant == null || combatant.m_Deleted || m_Mobile.m_Deleted ||
                        combatant.m_Map != m_Mobile.m_Map || !combatant.Alive || !m_Mobile.Alive ||
        ...
        public virtual void AggressiveAction(Mobile aggressor, bool criminal)
        {
            ...
            if (addAggressor)
            {
                m_Aggressors.Add(AggressorInfo.Create(aggressor, this,
            ...
            if (addAggressed)
            {
                aggressor.m_Aggressed.Add(AggressorInfo.Create(aggressor, this,
        ...
        public Target BeginTarget(int range, bool allowGround, TargetFlags flags, TargetStateCallback callback,
            object state)
        {
                            ...
                            for (int i = 0; i < oldSector.Items.Count; ++i)
                            {
                                ...
                                if (item.AtWorldPoint(oldX, oldY) &&
                                    ...
                                    return false;
                                         ...
                                         (item.Z == newZ ||
                                          item.Z + item.ItemData.Height > newZ && newZ + 15 > item.Z) &&
                                         !item.OnMoveOver(this))
                                    return false;
                            }
                        }
                                ...
                                foreach (NetState ns in eable)
                                {
                                        ...
                                        ns.Mobile.CanSee(root))
                                    {
                                        if (p == null)
                                        {
                                            ...
                                            p = Packet.Acquire(new DragEffect(src, from, item.ItemID, item.Hue,
                                                amount));
                                        }
        ...
        public virtual void SendDropEffect(Item item)
        {
            if (m_DragEffects && !item.Deleted)
            {
                ...
                if (map != null && (root == null || root is Item))
                {
                    ...
                    foreach (NetState ns in eable)
                    {
                        if (ns.Mobile != this && ns.Mobile.CanSee(this) && ns.Mobile.InLOS(this) &&
                            ns.Mobile.CanSee(root))
                        {
                            if (p == null)
                            {
                                // Use location instead of mobile, this way we can accurately check sameLoc for SA clients
                                IEntity src = new Entity(Serial.Zero,
                                    new Point3D(m_Location.X, m_Location.Y, m_Location.Z + 12), m_Map);
                                IEntity trg;
            ...
            if (m_Map != null)
            {
                ...
                for (int i = 0; i < hears.Count; ++i)
                {
                    ...
                    if (mutatedArgs == null || !CheckHearsMutatedSpeech(heard, mutateContext))
                    {
                        ...
                        {
                            if (regp == null)
                                regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, hue, 3, m_Language, Name,
                                    text));
                    ...
                    else
                    {
                        ...
                        {
                            if (mutp == null)
                                mutp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, hue, 3, m_Language, Name,
                                    mutatedText));
        ...
        public virtual void Damage(int amount, Mobile from, ElementalType type)
        {
            ...
            if (map != null)
            {
                ...
                foreach (NetState state in eable)
                {
                    if (state.Mobile.CanSee(this))
                    {
                        ...
                        //} else {
                        if (p == null)
                            p = Packet.Acquire(new MobileAnimation(this, action, frameCount, repeatCount, forward,
                                repeat, delay));
            ...
            if (oldLocation != newLocation)
            {
                ...
                if (map != null)
                {
                        ...
                        foreach (object o in eable)
                        {
                            if (o is Item)
                            {
                                ...
                                if (!Utility.InRange(oldLocation, loc, range) &&
                                    Utility.InRange(newLocation, loc, range) && CanSee(item))
                            ...
                            else if (o != this && o is Mobile)
                            {
                                ...
                                if (m.m_NetState != null &&
                                    (isTeleport && (!m.m_NetState.HighSeas || !m_NoMoveHS) || !inOldRange) &&
                                    m.CanSee(this))
                                {
                    ...
                    else
                    {
                        ...
                        foreach (NetState ns in eable)
                        {
                            if ((isTeleport && (!ns.HighSeas || !m_NoMoveHS) ||
        ...
        {
            Effects.SendMovingParticles(this, to, itemID, speed, duration, fixedDirection, explodes, effect,
                explodeEffect, explodeSound, unknown);
        }
        ...
        public virtual void ProcessDelta()
        {
            ...
            if ((delta & (MobileDelta.WeaponDamage | MobileDelta.Resistances | MobileDelta.Stat | MobileDelta.Weight |
                          MobileDelta.Gold | MobileDelta.Armor | MobileDelta.StatCap | MobileDelta.Followers |
                          MobileDelta.TithingPoints | MobileDelta.Race)) != 0)
            {
            ...
            if (m.m_Map != null && (sendRemove || sendIncoming || sendPublicStats || sendHits || sendMoving ||
                                    sendHair || sendFacialHair || sendHealthbarPoison || sendHealthbarYellow))
            {
                Mobile beholder;
            ...
            string args, bool noLineOfSight)
        {
            if (m_Map != null)
            {
                ...
                foreach (NetState state in eable)
                {
                    ...
                    {
                        if (p == null)
                            p = Packet.Acquire(new MessageLocalizedAffix(m_Serial, Body, type, hue, 3, number, Name,
                                affixType, affix, args));
        ...
        public void SendLocalizedMessage(int number, string args, int hue)
        {
            ...
            else
            {
                ...
                if (ns != null)
                    ns.Send(new MessageLocalized(Serial.MinusOne, -1, MessageType.Regular, hue, 3, number, "System",
                        args));
            }
        }
