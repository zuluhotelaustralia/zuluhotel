...
namespace Server.Accounting
{
	...
	public static string HashMD5( string phrase )
	{
	    ...
	    //this really ought to work... sorry in advance if it doesn't --sith
	    byte[] b_phrase = Encoding.Unicode.GetBytes(phrase);
	    byte[] hashcode = System.Security.Cryptography.MD5.Create().ComputeHash( b_phrase, 0, b_phrase.Length );

	    StringBuilder builder = new StringBuilder();

	    for(int i = 0; i < hashcode.Length; i++){
		builder.Append( hashcode[i] );
	    }
	...
	public static string HashSHA1( string phrase )
	{
	    ...
	    byte[] b_phrase = Encoding.Unicode.GetBytes(phrase);
	    byte[] hashcode = System.Security.Cryptography.SHA1.Create().ComputeHash( b_phrase, 0, b_phrase.Length );
	...
	public void SetPassword( string plainPassword )
	{
		...
		default: // PasswordProtection.NewCrypt
			...
			//are you fucking kidding me?  congrats on inventing the salted hash, I guess? -sith
			// todo:  change this to bcrypt or something that's actually secure and designed for storing secrets
