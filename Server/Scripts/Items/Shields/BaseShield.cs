...
namespace Server.Items
{
	...
	public override double ArmorRating
	{
		...
		if ( m != null )
		    return ( ( m.Skills[SkillName.Parry].Value * ar ) / 300.0 ) + 1.0;
		else
		    return ar;
	    }
	}
	...
	public override int OnHit( BaseWeapon weapon, int damage )
	{
	    ...
	    if( Core.Debug ) {
		Console.WriteLine( "Shield.OnHit: ar = {0}, chance = {1}", ar, chance );
	    }
	    
	    if( chance < 0.01 )
		chance = 0.01;
	    /*
	      FORMULA: Displayed AR = ((Parrying Skill * Base AR of Shield) ÷ 300) + 1 
	    ...
	    */
	    ...
	    if( Core.Debug ) {
		Console.WriteLine( "Checking Parry skill with chance = {0}", chance );
	    }
		...
		if( Core.Debug ) {
		    Console.WriteLine( "Successful Parry, damage = {0}", damage );
		}
		owner.FixedEffect( 0x37B9, 10, 16 );

		owner.Stam -= Utility.Random( 1, 3 );
