...
#region Factions
	...
	{
	    if ( type == StatType.Str )
		return BaseStrBonus + m_StrBonus; //Attributes.BonusStr;
	    else if ( type == StatType.Dex )
		return BaseDexBonus + m_DexBonus; //Attributes.BonusDex;
	    else
		return BaseIntBonus + m_IntBonus; //Attributes.BonusInt;
	}
	...
	public override void AddNameProperty( ObjectPropertyList list )
	{
		...
		int index = (int)m_ZuluSkillMods.Mod.Skill;
		string title = SkillInfo.Table[index].Title;
		
		list.Add(1053099, "#{0}\t{1}", title, GetNameString() );
	    }
	    else {
		switch ( m_Resource )
		{
