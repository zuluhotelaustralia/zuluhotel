...
namespace Server.Commands
{
    public class CommandHandlers
    {
        ...
        [Usage("Consider")]
        [Description("Gives the user indication of their sins and transgressions, toggles karma lock")]
        public static void Consider_OnCommand(CommandEventArgs e)
        {
            Mobile from = e.Mobile;
            bool inrange = false;
            foreach (IEntity ent in from.GetObjectsInRange(4))
            {
                if (ent is AnkhWest || ent is AnkhNorth || ent is ResGate)
                {
                    inrange = true;
                    break;
                }
            ...
            if (inrange)
            {
                ...
                pm.Say("I must consider my sins...");
                pm.SendMessage("You have committed {0} murders in recent memory.", from.ShortTermMurders);

                if (!pm.KarmaLocked)
                {
                    pm.KarmaLocked = true;
                    pm.SendLocalizedMessage(1060192); // Your karma has been locked. Your karma can no longer be raised.
                }
                else
                {
            ...
            {
                from.SendMessage("You must be near an Ankh to use this command.");
            }
        }
