...
namespace Server
{
    ...
    public static class Core
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("RunUO.T2A v{0}.{1} Alpha r{3} (based on RunUO 2.3 r1083)", ver.Major, ver.Minor,
                ver.Build, ver.MinorRevision);
            ...
            if (m_MultiProcessor || Is64Bit)
                Console.WriteLine("Core: Optimizing for {0} {2}processor{1}", m_ProcessorCount,
                    m_ProcessorCount == 1 ? "" : "s", Is64Bit ? "64-bit " : "");
    ...
    public class FileLogger : TextWriter, IDisposable
    {
        ...
        {
            m_FileName = file;
            using (StreamWriter writer = new StreamWriter(new FileStream(m_FileName,
                append ? FileMode.Append : FileMode.Create, FileAccess.Write, FileShare.Read)))
            {
                writer.WriteLine(">>>Logging started on {0}.",
                    DateTime.Now.ToString("f")); //f = Tuesday, April 10, 2001 3:51 PM 
            }
        ...
        public override void Write(char ch)
        {
            using (StreamWriter writer =
                new StreamWriter(new FileStream(m_FileName, FileMode.Append, FileAccess.Write, FileShare.Read)))
            {
                if (m_NewLine)
                {
